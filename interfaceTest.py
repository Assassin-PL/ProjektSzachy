...
from TCPserwer import TCPClient
...
class ChessScene(QGraphicsScene):
    ...
    def mousePressEvent(self, event):
        if self.time_left <= 0 : self.close_game()
    ...
    def mouseReleaseEvent(self, event):
        if self.selected_piece:
            ...
            czy_roszada,  czy_bic, new_position = self.manage_piece_collision(buffor, collision_piece_id, czy_roszada)
            self.manage_piece_mov(new_position, collision_piece_id, czy_bic, czy_roszada, czy_te_same_kolory)
            self.selected_piece = None
            # self.opposite_turn()
            ...
            self.timeInkrement = MAX_TIME
            self.is_king_exist()
            self.make_AI_move_old()

    def manage_piece_collision(self, buffor, collision_piece_id, czy_roszada=False,):
        if collision_piece_id is not None:
            ...
            if isinstance(collision_piece, ChessPiece):
                ...
                return czy_roszada, czy_bic, new_position

        else: return False, False, buffor

    def make_AI_move_old(self):
        if self.isAI and isinstance(self.logic, ChessLogic):
            pionek , ruch = self.logic.give_me_best_black_piece()
            if isinstance(pionek, ChessPiece):
                ruch = pionek.convert_to_chess_position(ruch[0], ruch[1])
        if pionek is not None and ruch is not None and isinstance(pionek, ChessPiece) and not self.logic.white_turn:
            print("znalazlem najlepszego pionka: ")
            # Użycie metody formatowania stringów do stworzenia nowego ciągu
            text_to_copy = "z {} {} do {} {}".format(pionek.position_in_chess[0], pionek.position_in_chess[1], ruch[0], ruch[1])
            slowo = "wykonano ruch do: " + text_to_copy
            self.opposite_turn((text_to_copy[2], text_to_copy[4]), (text_to_copy[-3], text_to_copy[-1]))
            print(slowo)
    ...
    def make_AI_move(self):
        if self.isAI and isinstance(self.logic, ChessLogic):
            ...
            if pionek is not None and ruch is not None and isinstance(pionek, ChessPiece) and not self.logic.white_turn:
                ...
                text_to_copy = "z {} {} do {} {}".format(pionek.position_in_chess[0], pionek.position_in_chess[1], ruch[0], ruch[1])
                slowo = "wykonano ruch do: " + text_to_copy
                # test bicia pionka dla mojej logiki
                new_position = ruch
                buffor = new_position
                collision_piece_id = self.logic.check_collision(new_position)  # Sprawdź kolizję z innym pionkiem
                czy_roszada = False
    ...
    def manage_piece_mov(self, new_position, collision_piece_id, czy_bic=False, czy_roszada=False, czy_te_same_kolory=False):
        if isinstance(self.selected_piece, ChessPiece):
            ...
            new_pos = self.selected_piece.pos()
            a = new_position[0] * SQUARE_SIZE + SQUARE_SIZE
            b = new_position[1] * SQUARE_SIZE + SQUARE_SIZE
            new_pos.setX(a)
            new_pos.setY(b)
    ...
    def opposite_turn(self, previous_position, new_position_in_chess):
        # greacz jest kolorem białym
        self.selected_piece = ChessPiece('test', 'cos', 14124)
        print("Test, jest tura przeciwnika")
        self.selected_piece = self.logic.get_piece(previous_position[0], previous_position[1])
        print(new_position_in_chess)
        print("moj pionek zaznaczony")
        new_position = self.selected_piece.convert_to_logic_position(new_position_in_chess[0], new_position_in_chess[1])
        print(new_position)
        buffor = new_position
        collision_piece_id = self.logic.check_collision(new_position)  # Sprawdź kolizję z innym pionkiem
        # czy_bic = False
    ...
    def save_history_to_sqlite(self, xml_file = 'history.xml', db_file='history.db'):
        # Parse the XML file
        ...
        for history in root.findall('history'):
            turn = history.find('turn').text if history.find('turn') is not None else None
            piece_type = history.find('piece_type').text if history.find('piece_type') is not None else None
            color = history.find('color').text if history.find('color') is not None else None
            position = history.find('position').text if history.find('position') is not None else None
            insert_data.append((turn, piece_type, color, position))

        # Use executemany to insert data
        cursor.executemany('''INSERT INTO history (turn, piece_type, color, position)
                              VALUES (?, ?, ?, ?)''', insert_data)

        # Commit changes and close connection
        conn.commit()
        conn.close()
        return "Data from XML has been inserted into the SQLite database successfully."
...
class ChessView(QMainWindow):
    ...
    def init_ui(self):
        ...
        # self.timerer = QTimer(self)
        # self.timerer.timeout.connect(self.check_response)
        # self.timerer.start(2000)  # Timer wyzwala co 1000 ms (1 sekunda)
    ...
    def on_connect_clicked(self):
        ...
        game_port = int(self.port_input.text())

        # Przypisz tekst do atrybutów sceny
        self.scene.IPadress = ip_address
        self.scene.gamePort = game_port

        if isinstance(self.scene.Klient, TCPClient):
            if self.scene.IPadress == self.scene.Klient.host_name and self.scene.gamePort == int(self.scene.Klient.port):
                print("Moge sie polaczyc z Serwerem")
                self.scene.Klient.connect()
                self.scene.Klient.send_message("Polaczylem sie")
                self.scene.Klient.server_connected = True
            else:
                print("Nie moge sie polaczyc z takim serwerem")
    ...
    def on_load_settings_clicked(self):
        ...
        if isinstance(self.scene.Klient, TCPClient):
            if self.scene.IPadress == self.scene.Klient.host_name and self.scene.gamePort == int(self.scene.Klient.port):
                ...
                self.scene.Klient.connect()
                self.scene.Klient.send_message("Polaczylem sie")
                print("Otrzymalem wiadomosc:" + self.scene.Klient.receive_message())
                self.scene.Klient.server_connected = True
            else:
                print("Nie moge sie polaczyc z takim serwerem")
    ...
    def set_chess_central_scene(self):
        ...
        print("Czuy wlaczona AI: " )
        print(self.scene.isAI)
    ...
    def on_copy_button_clicked(self):
        ...
        print(text_to_copy)
        # np(z a 2 do a 3)
        self.scene.opposite_turn((text_to_copy[2], text_to_copy[4]), (text_to_copy[-3], text_to_copy[-1]))
        if self.scene.Klient.server_connected:
            self.scene.Klient.send_message(text_to_copy)

    def check_response(self):
        if self.scene.Klient.server_connected:
            # Tutaj sprawdzamy responsa od TCP klienta lub wysyłamy zapytanie do serwera
            response = self.scene.Klient.receive_message()  # Przykładowa metoda odbierająca wiadomość od klienta
    ...
    # do zrobienia 1. wywala roszada ale tylko dla ruchu z pola tekstowego ( jolo nie wiem o co chodzi xdxd) 2. historia z history.json i jakis odtwarzacz z buttona by sie przydal xd
